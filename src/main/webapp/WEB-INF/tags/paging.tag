...
<c:if test="${pagedListHolder.pageCount > 1}">
    ...
    <ul class="pagination">
        <c:if test="${!pagedListHolder.firstPage}">
            <li class="page-item"><a class="page-link" class="page-link" href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage()-1))%>"><spring:message code="pagination_previous"/></a></li>
        </c:if>
        <c:if test="${pagedListHolder.firstLinkedPage > 0}">
            <li><a class="page-link" href="<%=StringUtils.replace(pagedLink,"~","0")%>"></a></li>
        </c:if>
        <c:if test="${pagedListHolder.firstLinkedPage > 1}">
            <li><span class="page-item"><spring:message code="pagination_next"/></span></li>
        </c:if>
        ...
        <c:if test="${!pagedListHolder.lastPage}">
            <li class="page-item"><a class="page-link" href="<%=StringUtils.replace(pagedLink,"~", String.valueOf(pagedListHolder.getPage() + 1))%>"><spring:message code="pagination_next"/></a> </li>
        </c:if>
    </ul>
    </nav>
</c:if>